<template>
  <div class="contentPage">
    <!--<profilebar />-->
    <v-toolbar :title="data?.showcase?.name" :style="`background: ${data?.showcase?.color}`">
    ...
    <section data-bs-version="5.1" class="header7 cid-twaHyROL0Y mbr-fullscreen" id="header7-43"
      :style="`background: ${data?.showcase?.color}`">
      ...
      <div class="container">
        <div class="row align-items-center">
          <div class="col-md-6 align-left img-block"
            :style="`background-color: ${data?.showcase?.color};`">
            <div class="mbr-figure">
              <img :src="`${data?.showcase?.image?.url}`"
                :alt="data?.showcase?.name">
            </div>
          </div>
          <div class="mbr-white col-md-6" :style="`color: ${data?.showcase?.colortext} !important`">
            ...
            <h4 class="mbr-section-subtitle mbr-fonts-style align-left pb-2 display-5"
              :style="`color: ${data?.showcase?.colortext} !important`">
              {{ data?.showcase?.description?.html }}</h4>
            <h6 class="mbr-section-title mbr-white mbr-bold mbr-fonts-style align-left display-5"
              :style="`color: ${data?.showcase?.colortext} !important`">Created:
              {{ new Date(data?.showcase?.created_at).toLocaleDateString() }}</h6>
          </div>
        </div>
      </div>
    </section>
    ...
    <v-card>
      ...
      <v-tabs-window v-model="tab">
        <v-tabs-window-item>
          <v-container fluid>
            <div style="padding-top: 10px;" v-for="products in data?.showcase?.products?.items"
              :key="products.id">
